...
> 海关来源供应商的 veto_non_manufacturer 使用严格证据门槛：海关记录只能证明企业曾作为出口商或发货人，不能证明企业自身生产。系统必须根据天眼查经营范围同时确认"具备实际制造能力"和"制造对象与当前采购产品匹配"后才允许继续评分；经营范围缺失、制造对象不相关、AI返回字段不完整或AI调用失败时，均触发非制造商一票否决。该规则按当前采购产品动态判断，不写死家具、电子、机械等行业词，并兼容零部件、原材料、模具和加工服务需求。

> 天眼查MCP请求失败与"确实未找到"严格区分：`_call`方法对429频率限制、5xx服务器错误和网络超时自动重试3次（间隔3/6/9秒）；`search_companies`请求失败时返回`None`（区别于空列表`[]`表示确实没找到）；`query_supplier_full_data`收到`None`时返回`tyc_match_status="error"`；初筛引擎遇到`error`状态时跳过该供应商（不写初筛结论、不判0分、不改供应商状态），下次重新初筛时再查。这避免了批量初筛时天眼查频率限制导致大量供应商被误判为"未找到同名企业"而淘汰的问题。
